...
function Server (opts) {
  ...
  if(self.manager && self.manager.log)
...
Server.prototype.prep = function(callback) {
  ...
  self.req({method:'PUT', uri:self.resource}, function(er, resp, body) {
    ...
    var ddoc = { "_id": "_design/socket.io" };
    ...
    self.req(url, function(er, resp, body) {
      ...
      //
      // Store some testing code for now.
      //

      function attach(filename, type, data) {
        if(! (data instanceof Buffer))
          data = new Buffer(data);

        ddoc._attachments = ddoc._attachments || {};
        ddoc._attachments[filename] = {
            'content_type': type
      ...
      var mgr = self.manager
        ...
        , hs = [
              id
            , mgr.enabled('heartbeats') ? mgr.get('heartbeat timeout') || '' : ''
            , mgr.get('close timeout') || ''
            , 'couchdb' // transport
          ].join(':');
      attach('1', 'text/plain', hs); // Version 1 handshake

      attach('socket.io.html', 'text/html; charset=utf-8', [
          '<!DOCTYPE html>'
        , '<html>'
        ,   '<head>'
        ,     '<title>Socket App</title>'
        ,     '<script src="http://ajax.googleapis.com/ajax/libs/jquery/1.6.4/jquery.min.js"></script>'
        ,     '<script src="/socket_io/_design/socket.io/socket.io.js"></script>'
        ,   '</head>'
        ,   '<body>'
        ,     '<h1>Socket.IO</h1>'
        ,     '<div id="j"></div>'
        ,     '<script src="/socket_io/_design/socket.io/test.js"></script>'
        ,   '</body>'
        , '</html>'
        ,
        ].join(''));

      var test_js = function(win) {
        var J = $('#j');

        //io.set('log level', 20);
        var S = io.connect();

        S.on('pong', function(data) { console.log('PONG ' + data) });

        function ping(arg) {
          return S.emit('ping', arg || new Date);
        }

        function R() {
          return S.emit('restart', function(result) { console.log('Restart: ' + result) });
        }
      ...
      test_js = '(' + test_js.toString() + ')(window);';
      attach('test.js', 'application/javascript; charset=utf8', test_js);

      var client_js_path = '/socket.io.js';
      var result = self.manager.static.has(client_js_path);
      if(!result || !result.callback)
        return callback(new Error('Cannot find manager static ' + client_js_path));

      result.callback(client_js_path, function(er, content, id) {
        if(er)
          return callback(er);

        var type = result.mime.type + '; charset=' + result.mime.encoding;
        attach('socket.io.js', type, content);
